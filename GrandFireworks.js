...
(function (global) {
  ...
  class GrandFireworks extends EventTarget {
    ...
    setColorTheme(name){const t=COLOR_THEMES[name];if(!t)return this;this.setOptions({colorTheme:name});if(t.palettes!=='default'){const pals=Array.isArray(t.palettes)?[...t.palettes]:t.palettes;this.options.show.palettes=pals;const palette=this._palette();if(palette&&palette.length){const colors=palette.map(h=>{const c=h.replace('#','');const n=parseInt(c.length===3?c.split('').map(x=>x+x).join(''):c,16);return[(n>>16&255)/255,(n>>8&255)/255,(n&255)/255];});this.particles.forEach((p,i)=>{const c=colors[i%colors.length];p.r=c[0];p.g=c[1];p.b=c[2];});this.rockets.forEach(r=>{r.colors=palette;});}}return this;}
