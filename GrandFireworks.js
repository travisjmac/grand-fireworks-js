...
(function (global) {
  ...
  const PRESETS = {
    low:    { fps:30, dprCap:1,    maxParticles:1500, maxRockets:4,  launchInterval:1000, particleScale:.72, secondary:.45 },
    medium: { fps:60, dprCap:1.25, maxParticles:3000, maxRockets:6,  launchInterval:750,  particleScale:.88, secondary:.70 },
    high:   { fps:60, dprCap:1.5,  maxParticles:5000, maxRockets:10, launchInterval:550,  particleScale:1,   secondary:1 },
    ultra:  { fps:60, dprCap:2,    maxParticles:8000, maxRockets:14, launchInterval:350,  particleScale:1.2, secondary:1.25 }
  };
  const STYLES = {
    classic:{
    ...
    bold:{
      visuals:{ opacity:1, trails:true, trailFade:.06, bloom:1.9, rocketExhaust:true, explosionFlashes:true, starChance:.15, groupedSalvos:true, secondaryCrackle:true },
      show:{ intensity:1.6, openingSalvo:9, launchInterval:350, launchSpread:.75, angleRange:22, angleStrength:1.6, enabledTypes:'all', palettes:[['#FF0055','#FFCC00'],['#00F2FE','#4FACFE'],['#FFD700','#FFA500','#FFFFFF'],['#FF1744','#FF5252','#FF8A80','#FFFFFF'],['#AA00FF','#D17FE0','#EA80FC','#FFFFFF']] }
    },
    spectacle:{
      visuals:{ opacity:1, trails:true, trailFade:.18, bloom:2.2, rocketExhaust:true, explosionFlashes:true, starChance:.18, groupedSalvos:true, secondaryCrackle:true },
      show:{ intensity:2, openingSalvo:12, launchInterval:250, launchSpread:.8, angleRange:26, angleStrength:1.8, enabledTypes:'all', palettes:[['#FF0055','#FFCC00'],['#00F2FE','#4FACFE'],['#FFD700','#FFA500','#FFFFFF'],['#FF1744','#FF5252','#FF8A80','#FFFFFF'],['#AA00FF','#D17FE0','#EA80FC','#FFFFFF'],['#00E676','#69F0AE','#B9F6CA','#FFFFFF']] }
    }
  };
  const COLOR_THEMES = {
    default:{ label:'Default', palettes:'default' },
    iceBlue:{ label:'Ice Blue', palettes:[['#00BFFF','#E0FFFF'],['#1E90FF','#87CEEB','#FFFFFF'],['#00CED1'],['#4169E1','#00BFFF','#E0FFFF','#FFFFFF'],['#87CEFA','#B0E0E6','#F0F8FF']] },
    emberRed:{ label:'Ember Red', palettes:[['#FF4500','#FFD700'],['#DC143C','#FF6347','#FFFFFF'],['#FF0000'],['#8B0000','#FF4500','#FFD700','#FFFFFF'],['#FF6347','#FF7F50','#FFE4E1']] },
    neonGreen:{ label:'Neon Green', palettes:[['#00FF7F','#7FFF00'],['#32CD32','#ADFF2F','#FFFFFF'],['#00FF00'],['#006400','#00FF7F','#7FFF00','#FFFFFF'],['#7CFC00','#98FB98','#F0FFF0']] },
    goldenSun:{ label:'Golden Sun', palettes:[['#FFD700','#FFA500'],['#FFD700','#FFFACD','#FFFFFF'],['#FFC125'],['#B8860B','#FFD700','#FFE4B5','#FFFFFF'],['#F0E68C','#FFF8DC','#FAFAD2']] },
    royalPurple:{ label:'Royal Purple', palettes:[['#8A2BE2','#DDA0DD'],['#9400D3','#BA55D3','#FFFFFF'],['#9932CC'],['#4B0082','#8A2BE2','#DA70D6','#FFFFFF'],['#9370DB','#D8BFD8','#E6E6FA']] },
    festival:{ label:'Festival Mix', palettes:[['#FF0055','#FFCC00'],['#00F2FE','#4FACFE'],['#FFD700','#FFA500','#FFFFFF'],['#FF1493','#FF69B4','#FFC0CB','#FFFFFF'],['#AA00FF','#D17FE0','#EA80FC','#FFFFFF']] }
  };
  const DEFAULTS = {
    container: null, mode:'fullscreen', placement:'overlay', clip:true, zIndex:9999,
    autoStart:false, baseStyle:'medium', colorTheme:'default', showFps:false, duration:0, durationMode:'graceful', maxFinishTime:5000,
    background:false,
  ...
  class GrandFireworks extends EventTarget {
    constructor(options={}) {
      super(); this.userOptions=options; this.options=this._resolve(options); this.state='idle'; this.rockets=[];this.pendingRockets=[];this.particles=[];this.pool=[];this.flashes=[];this.textBlocks=[];this.textSequence=null;this.textSequenceId=0;this.userPaused=false;this.autoPauseReasons=new Set();this.reducedMotion=false;this.quality=1;this.contextLossCount=0;this.finalePlayed=false;this.lastLaunch=0;this.elapsed=0;this.raf=0;this.stopPromise=null;this.resizePending=true;this.fps=0;this.nextAdapt=0;
      this.container=cssTarget(this.options.container)||document.body;this._buildLayers();this._initRenderer();this._resize();this._bind();if(this.options.autoStart)requestAnimationFrame(()=>this.start());
    }
    _resolve(input={}){const styleName=input.baseStyle||'medium',style=STYLES[styleName]||STYLES.medium;let o=merge(merge(DEFAULTS,style),input);const themeName=o.colorTheme||'default',theme=COLOR_THEMES[themeName];if(theme&&theme.palettes!=='default')o.show.palettes=theme.palettes;const name=(o.performance&&o.performance.preset)||'high',p=PRESETS[name]||PRESETS.high;o.performance=merge({preset:name,adaptive:true,pauseWhenHidden:true,pauseWhenOffscreen:true,respectReducedMotion:true},o.performance||{});o.visuals.trailFade=clamp(Number(o.visuals.trailFade)||.115,.03,1);o.visuals.bloom=clamp(Number(o.visuals.bloom)||1,.5,2);o.visuals.starChance=clamp(Number(o.visuals.starChance)||0,0,.3);o.renderer.preserveDrawingBuffer=o.renderer.preserveDrawingBuffer==='auto'?(o.mode==='contained'||Boolean(o.visuals.trails)):Boolean(o.renderer.preserveDrawingBuffer);o.show.maxParticles=clamp(Number(o.show.maxParticles||p.maxParticles),100,10000);o.show.maxRockets=clamp(Number(o.show.maxRockets||p.maxRockets),1,25);o.show.launchInterval=Number(o.show.launchInterval||p.launchInterval);o.show.launchSpread=clamp(Number(o.show.launchSpread),0,1);o.show.angleRange=clamp(Number(o.show.angleRange),0,45);o.show.angleStrength=clamp(Number(o.show.angleStrength),0,3);o.show.textRocketAngle=clamp(Number(o.show.textRocketAngle),-45,45);const fps=Number(o.performance.fps),dprCap=Number(o.performance.dprCap),particleScale=Number(o.performance.particleScale),secondary=Number(o.performance.secondary);o.performance.fps=fps>0?fps:p.fps;o.performance.dprCap=dprCap>0?dprCap:p.dprCap;o.performance.particleScale=particleScale>0?particleScale:p.particleScale;o.performance.secondary=secondary>=0?secondary:p.secondary;if((o.renderer.preferred==='canvas2d')||(o.renderer.preferred==='auto'&&o.mode==='contained'))o.show.maxParticles=Math.round(o.show.maxParticles/2);o.finale.type=o.finale.type==='super-grand-finale'?o.finale.type:'super-grand-finale';o.finale.trails=clamp(Math.round(Number(o.finale.trails)||10),3,20);o.finale.trailFlight=Math.max(500,Number(o.finale.trailFlight)||1100);o.finale.burstScale=clamp(Number(o.finale.burstScale)||1,.4,2);o.finale.maxWaitBeforeLaunch=Math.max(0,Number(o.finale.maxWaitBeforeLaunch)||0);o.finale.particleScale=clamp(Number(o.finale.particleScale)||1,.25,2);o.finale.finishDelay=Math.max(0,Number(o.finale.finishDelay)||0);o.finale.maxDuration=Math.max(1000,Number(o.finale.maxDuration)||9000);o.duration=Math.max(0,Number(o.duration)||0);return o;}
    _buildLayers(){const o=this.options,c=this.container;this.root=document.createElement('div');this.root.className='grand-fireworks-root';Object.assign(this.root.style,{position:o.mode==='fullscreen'?'fixed':'absolute',inset:'0',width:'100%',height:'100%',overflow:o.clip?'hidden':'visible',pointerEvents:'none',isolation:'isolate',zIndex:String(o.placement==='background'?0:o.zIndex),opacity:String(clamp(Number(o.visuals.opacity ?? 1), 0, 1))});if(o.mode==='contained'&&getComputedStyle(c).position==='static'){this.oldPosition=c.style.position;c.style.position='relative';}this.backdrop=document.createElement('div');Object.assign(this.backdrop.style,{position:'absolute',inset:'0',opacity:'0',transition:`opacity ${o.transition.fadeIn}ms ${o.transition.easing}`});if(o.background){this.backdrop.style.background=o.background.value||'';this.backdrop.style.opacity='0';if(o.background.className)this.backdrop.className=o.background.className;}this.canvas=this._canvas();this.textCanvas=this._canvas();this.textCanvas.className='grand-fireworks-text-layer';this.textCanvas.style.display='none';this.textCtx=this.textCanvas.getContext('2d',{alpha:true,desynchronized:true});this.root.append(this.backdrop,this.textCanvas,this.canvas);if(o.placement==='background')c.insertBefore(this.root,c.firstChild);else c.appendChild(this.root);if(o.showFps){this.fpsEl=document.createElement('div');Object.assign(this.fpsEl.style,{position:'fixed',zIndex:'2147483647',top:'8px',left:'8px',font:'600 13px/1 ui-monospace,monospace',color:'rgba(255,255,255,.6)',background:'rgba(0,0,0,.5)',padding:'4px 8px',borderRadius:'6px',pointerEvents:'none',userSelect:'none',backdropFilter:'blur(4px)'});document.body.appendChild(this.fpsEl);}}
    ...
    _bind(){this.onResize=()=>this.resizePending=true;this.onVisibility=()=>{if(this.options.performance.pauseWhenHidden&&document.hidden)this._pauseFor('hidden');else this._resumeFor('hidden');};window.addEventListener('resize',this.onResize,{passive:true});document.addEventListener('visibilitychange',this.onVisibility);if(this.options.mode==='contained'&&global.ResizeObserver){this.resizeObserver=new ResizeObserver(this.onResize);this.resizeObserver.observe(this.container);}if(global.IntersectionObserver){this.intersectionObserver=new IntersectionObserver(entries=>{const visible=entries.some(entry=>entry.isIntersecting&&entry.intersectionRatio>0);if(this.options.performance.pauseWhenOffscreen&&!visible)this._pauseFor('offscreen');else this._resumeFor('offscreen');},{threshold:.01});this.intersectionObserver.observe(this.options.mode==='contained'?this.container:this.root);}if(global.matchMedia){this.motionQuery=global.matchMedia('(prefers-reduced-motion: reduce)');this.reducedMotion=Boolean(this.motionQuery.matches);this.onMotionChange=event=>this.reducedMotion=Boolean(event.matches);if(this.motionQuery.addEventListener)this.motionQuery.addEventListener('change',this.onMotionChange);else if(this.motionQuery.addListener)this.motionQuery.addListener(this.onMotionChange);}}
    start(run={}){if(this.state==='running')return this;clearTimeout(this.fadeTimer);this.runtimeDuration=run.duration===undefined?this.options.duration:Math.max(0,Number(run.duration)||0);this.elapsed=0;this.accepting=true;this.finalePlayed=false;this.userPaused=false;this.state='running';this.lastTime=performance.now();this.lastLaunch=this.lastTime;this.root.style.display='block';this.root.style.opacity=String(clamp(Number(this.options.visuals.opacity??1),0,1));this._audio();requestAnimationFrame(()=>{if(this.options.background)this.backdrop.style.opacity=String(this.options.background.opacity===undefined?1:this.options.background.opacity);});const opening=this._motionReduced()?Math.max(1,Math.round(this.options.show.openingSalvo*.35)):this.options.show.openingSalvo;for(let i=0;i<opening;i++)this._queueRocket(this.lastTime+i*180);if(this.autoPauseReasons.size){this.pausedState='running';this.state='paused';}else this._loop();this.dispatchEvent(new Event('start'));return this;}
    _motionReduced(){return this.options.performance.respectReducedMotion&&this.reducedMotion;}
    _pauseEngine(){if(!['running','finishing','finale','manual'].includes(this.state))return false;this.pausedState=this.state;this.state='paused';cancelAnimationFrame(this.raf);this.raf=0;return true;}
    _resumeEngine(){if(this.state!=='paused'||this.userPaused||this.autoPauseReasons.size)return false;this.state=this.pausedState||'running';this.lastTime=performance.now();this._loop();return true;}
    _pauseFor(reason){this.autoPauseReasons.add(reason);this._pauseEngine();return this;}
    _resumeFor(reason){this.autoPauseReasons.delete(reason);this._resumeEngine();return this;}
    pause(internal=false){if(internal)return this._pauseFor(typeof internal==='string'?internal:'internal');this.userPaused=true;const changed=this._pauseEngine();if(changed)this.dispatchEvent(new Event('pause'));return this;}
    resume(internal=false){if(internal)return this._resumeFor(typeof internal==='string'?internal:'internal');this.userPaused=false;const changed=this._resumeEngine();if(changed)this.dispatchEvent(new Event('resume'));return this;}
    ...
    destroy(){cancelAnimationFrame(this.raf);clearTimeout(this.fadeTimer);this.cancelTextSequence({clear:false});window.removeEventListener('resize',this.onResize);document.removeEventListener('visibilitychange',this.onVisibility);if(this.resizeObserver)this.resizeObserver.disconnect();if(this.intersectionObserver)this.intersectionObserver.disconnect();if(this.motionQuery&&this.onMotionChange){if(this.motionQuery.removeEventListener)this.motionQuery.removeEventListener('change',this.onMotionChange);else if(this.motionQuery.removeListener)this.motionQuery.removeListener(this.onMotionChange);}this.renderer.destroy();this.root.remove();if(this.fpsEl){this.fpsEl.remove();this.fpsEl=null;}if(this.audioContext&&this.audioContext.state!=='closed'){const closing=this.audioContext.close();if(closing&&closing.catch)closing.catch(()=>{});}this.audioContext=null;if(this.oldPosition!==undefined)this.container.style.position=this.oldPosition;this.state='destroyed';}
    _activateManual(state='manual'){if(this.state==='destroyed'||this.state==='paused')return false;const inactive=['idle','stopped','fading'].includes(this.state);if(!inactive)return false;clearTimeout(this.fadeTimer);cancelAnimationFrame(this.raf);this.raf=0;this.stopPromise=null;this.stopResolve=null;this.accepting=false;this.state=state;this.lastTime=performance.now();this.root.style.display='block';this.root.style.opacity=String(clamp(Number(this.options.visuals.opacity??1),0,1));if(this.options.background)this.backdrop.style.opacity=String(this.options.background.opacity===undefined?1:this.options.background.opacity);this.resizePending=true;if(this.autoPauseReasons.size){this.pausedState=state;this.state='paused';}else this._loop();return true;}
    ...
    launchFinale(options={}){const standalone=this._activateManual('finale');this.finalePlayed=true;this.finaleStarted=performance.now();this.finaleFinishedAt=0;this.pendingRockets.length=0;if(this.rockets.length>=this.options.show.maxRockets)this.rockets.length=0;this._createRocket({type:'grand-finale-carrier',x:.5,burstHeight:.24,finale:true,colors:['#FFFFFF','#FFD700','#00FFFF']});if(standalone||options.stopAfter!==false){this.accepting=false;this.state='finale';}this.dispatchEvent(new CustomEvent('finalestage',{detail:{stage:'carrier',type:this.options.finale.type}}));return this;}
    launchText(text,overrides={}){const cfg=merge(this.options.textFirework,overrides);if(!cfg.enabled||!text)return Promise.resolve([]);this._activateManual('manual');let value=String(text).trim();if(value.length>cfg.maxCharacters)value=value.slice(0,cfg.maxCharacters-1)+'…';const lines=this._wrapText(value,cfg);if(cfg.exclusive)this.accepting=false;const now=performance.now(),plans=this._textPlans(lines,cfg),sync=now+1800,total=plans.reduce((n,p)=>n+p.points.length,0),reserved=Math.max(1,Math.floor(this.options.show.maxParticles*.9)),stride=Math.max(1,Math.ceil(total/reserved));if(stride>1)for(const plan of plans)plan.points=plan.points.filter((_,i)=>i%stride===0);const needed=plans.reduce((n,p)=>n+p.points.length,0),keep=Math.max(0,this.options.show.maxParticles-needed);while(this.particles.length>keep){const removed=this.particles.shift();if(removed)this.pool.push(removed);}const latestSync=sync+(cfg.synchronizeExplosions?0:Math.max(0,plans.length-1)*250);if(cfg.exclusive){this.textReservedUntil=latestSync+cfg.revealDuration+cfg.holdDuration+cfg.dissolveDuration+cfg.fallDuration;this.textExclusiveUntil=this.textReservedUntil;}for(let i=0;i<plans.length;i++){const plan=plans[i],syncAt=sync+(cfg.synchronizeExplosions?0:i*250),travel=(this.height+30-plan.y)/.62;this.pendingRockets.push({launchAt:syncAt-travel,textPlan:plan,type:'text',x:this.width/2,y:this.height+25,burstY:plan.y,syncAt,colors:cfg.colors,cfg});}this.dispatchEvent(new CustomEvent('textlaunch',{detail:{text:value,lines,particles:needed}}));return Promise.resolve(lines);}
    _clearTextEffects(){this.pendingRockets=this.pendingRockets.filter(r=>!r.textPlan);this.rockets=this.rockets.filter(r=>r.type!=='text');for(let i=this.particles.length-1;i>=0;i--)if(this.particles[i].text)this.pool.push(this.particles.splice(i,1)[0]);this.textBlocks.length=0;this.textReservedUntil=0;this.textExclusiveUntil=0;if(this.state==='running')this.accepting=true;if(this.textCtx){this.textCtx.clearRect(0,0,this.textCanvas.width,this.textCanvas.height);this.textCanvas.style.display='none';}}
    cancelTextSequence(options={}){const sequence=this.textSequence;if(!sequence)return false;sequence.cancelled=true;clearTimeout(sequence.timer);this.textSequence=null;if(options.clear!==false)this._clearTextEffects();const result={status:'cancelled',completed:sequence.completed,total:sequence.items.length};sequence.resolve(result);this.dispatchEvent(new CustomEvent('textsequencecancel',{detail:result}));return true;}
    launchTextSequence(messages,options={}){const source=Array.isArray(messages)?messages:[messages],items=source.map(item=>typeof item==='object'&&item!==null?{text:String(item.text||'').trim(),overrides:merge(options.overrides||{},item.overrides||{}),duration:Number(item.duration)||0}:{text:String(item||'').trim(),overrides:merge({},options.overrides||{}),duration:0}).filter(item=>item.text);if(!items.length)return Promise.resolve({status:'completed',completed:0,total:0});const cancelled=this.cancelTextSequence({clear:options.clearExisting!==false});if(!cancelled&&options.clearExisting!==false)this._clearTextEffects();const sequence={id:++this.textSequenceId,items,completed:0,cancelled:false,timer:0,resolve:null};this.textSequence=sequence;return new Promise(resolve=>{sequence.resolve=resolve;const finish=()=>{if(sequence.cancelled)return;this.textSequence=null;if(options.clearOnComplete)this._clearTextEffects();const result={status:'completed',completed:sequence.completed,total:items.length};resolve(result);this.dispatchEvent(new CustomEvent('textsequenceend',{detail:result}));};const next=()=>{if(sequence.cancelled)return;if(sequence.completed>=items.length){finish();return;}if(sequence.completed>0&&options.clearBetween!==false)this._clearTextEffects();const index=sequence.completed,item=items[index],cfg=merge(this.options.textFirework,item.overrides);this.launchText(item.text,item.overrides);sequence.completed++;this.dispatchEvent(new CustomEvent('textsequenceitem',{detail:{index,total:items.length,text:item.text}}));const stagger=cfg.synchronizeExplosions?0:Math.max(0,cfg.maxLines-1)*250,automatic=1800+stagger+cfg.revealDuration+cfg.holdDuration+cfg.dissolveDuration+(Number(options.gap)||0),delay=item.duration||Number(options.interval)||automatic;sequence.timer=setTimeout(next,Math.max(0,delay));};this.dispatchEvent(new CustomEvent('textsequencestart',{detail:{total:items.length}}));const startDelay=Math.max(0,Number(options.startDelay)||0);if(startDelay)sequence.timer=setTimeout(next,startDelay);else next();});}
    ...
    _audio(){if(!this.options.sound.enabled)return null;const Audio=window.AudioContext||window.webkitAudioContext;if(!Audio)return null;try{if(!this.audioContext||this.audioContext.state==='closed')this.audioContext=new Audio();if(this.audioContext.state==='suspended'){const resumed=this.audioContext.resume();if(resumed&&resumed.catch)resumed.catch(()=>{});}return this.audioContext;}catch(e){return null;}}
    _playSound(type){const vol=clamp(Number(this.options.sound.volume),0,1);if(!this.options.sound.enabled||vol<=0)return;const a=this._audio();if(!a)return;try{if(type==='launch'){const mg=a.createGain();mg.connect(a.destination);mg.gain.setValueAtTime(vol*.25,a.currentTime);mg.gain.exponentialRampToValueAtTime(.001,a.currentTime+.5);const noise=a.createBufferSource(),nb=a.createBuffer(1,a.sampleRate*.5,a.sampleRate),nd=nb.getChannelData(0);for(let i=0;i<nd.length;i++)nd[i]=Math.random()*2-1;noise.buffer=nb;const nf=a.createBiquadFilter();nf.type='bandpass';nf.frequency.setValueAtTime(800,a.currentTime);nf.frequency.exponentialRampToValueAtTime(200,a.currentTime+.5);nf.Q.value=1.5;noise.connect(nf);nf.connect(mg);noise.start();noise.stop(a.currentTime+.5);}else if(type==='explode'){const gg=a.createGain();gg.connect(a.destination);gg.gain.setValueAtTime(0,a.currentTime);gg.gain.setValueAtTime(vol*.35,a.currentTime+.01);gg.gain.exponentialRampToValueAtTime(vol*.12,a.currentTime+.06);gg.gain.exponentialRampToValueAtTime(.001,a.currentTime+.5);const crack=a.createBufferSource(),cb=a.createBuffer(1,a.sampleRate*.06,a.sampleRate),cd=cb.getChannelData(0);for(let i=0;i<cd.length;i++)cd[i]=Math.random()*2-1;crack.buffer=cb;const cf=a.createBiquadFilter();cf.type='highpass';cf.frequency.value=2000;crack.connect(cf);cf.connect(gg);crack.start();const boom=a.createBufferSource(),bb=a.createBuffer(1,a.sampleRate*.5,a.sampleRate),bd=bb.getChannelData(0);for(let i=0;i<bd.length;i++)bd[i]=Math.random()*2-1;boom.buffer=bb;const bf=a.createBiquadFilter();bf.type='lowpass';bf.frequency.setValueAtTime(300,a.currentTime);bf.frequency.exponentialRampToValueAtTime(40,a.currentTime+.4);const bg=a.createGain();bg.gain.setValueAtTime(vol*.3,a.currentTime);bg.gain.exponentialRampToValueAtTime(.001,a.currentTime+.4);boom.connect(bf);bf.connect(bg);bg.connect(a.destination);boom.start();boom.stop(a.currentTime+.5);crack.stop(a.currentTime+.08);}}catch(e){}}
    _createRocket(o={}){if(this.rockets.length>=this.options.show.maxRockets)return;const show=this.options.show,colors=o.colors||this._palette(),type=o.type||this._type(),x=o.x===undefined?.5+(Math.random()-.5)*show.launchSpread:o.x,burstY=this.height*(o.burstHeight===undefined?.12+Math.random()*.38:o.burstHeight),startY=this.height+24,remaining=o.syncAt?Math.max(.2,(o.syncAt-performance.now())/1000):0,vy=o.syncAt?-(startY-burstY)/remaining:-(560+Math.random()*180),range=o.finale?18:show.angleRange,angle=o.angle===undefined?(o.syncAt?show.textRocketAngle:(Math.random()*2-1)*range):Number(o.angle)||0,vx=Math.abs(vy)*Math.tan(angle*Math.PI/180)*show.angleStrength;this._playSound('launch');this.rockets.push({x:x<=1?this.width*x:x,y:startY,vx,vy,burstY,type,colors,finale:o.finale,textPlan:o.textPlan,cfg:o.cfg,angle,sparkClock:0});}
    ...
    _loop(){if(this.raf||['stopped','idle','paused','destroyed'].includes(this.state))return;const frame=now=>{this.raf=0;if(['stopped','paused','destroyed'].includes(this.state))return;if(this.resizePending)this._resize();const dt=Math.min(.05,(now-this.lastTime)/1000||.016);this.lastTime=now;this.fps+=((1/dt)-this.fps)*.05;if(this.state==='running'){this.elapsed+=dt*1000;if(this.runtimeDuration>0&&this.elapsed>=this.runtimeDuration){this.accepting=false;if(this.options.durationMode==='strict'){this._fade(false);}else{this.state='finishing';this.finishStarted=now;this.wantFinale=this._finaleTriggered('duration');this.stopPromise=new Promise(resolve=>this.stopResolve=resolve);}}}
        ...
        this._update(now,dt);const trailFade=this.options.visuals.trails?1-Math.pow(1-this.options.visuals.trailFade,dt*60):1;this.renderer.render(this._drawItems(now),this.width,this.height,trailFade);this._drawText(now);if(this.fpsEl)this.fpsEl.textContent=`${Math.round(this.fps)} fps · ${this.particles.length} particles · ${this.rendererType}`;this._finish(now);this.raf=requestAnimationFrame(frame);};this.raf=requestAnimationFrame(frame);}
    ...
    _update(now,dt){if(this.state==='running'&&this.textExclusiveUntil&&now>=this.textExclusiveUntil){this.textExclusiveUntil=0;this.accepting=true;}for(let i=this.pendingRockets.length-1;i>=0;i--){const q=this.pendingRockets[i];if(now>=q.launchAt){this.pendingRockets.splice(i,1);this._createRocket(q.textPlan?{type:'text',x:.5,burstHeight:q.textPlan.y/this.height,colors:q.colors,textPlan:q.textPlan,cfg:q.cfg,syncAt:q.syncAt}:{});}}
      const motionScale=this._motionReduced()?.35:1;if(this.accepting&&now-this.lastLaunch>this.options.show.launchInterval/(this.options.show.intensity*motionScale)){const load=this.particles.length/Math.max(1,this.options.show.maxParticles*this.quality*motionScale),grouped=!this._motionReduced()&&this.options.visuals.groupedSalvos,count=grouped&&load<.58?1+Math.floor(Math.random()*3):1;for(let i=0;i<count;i++)this._createRocket();if(grouped&&load<.48&&Math.random()<.5){this._queueRocket(now+300+Math.random()*450);if(Math.random()<.45)this._queueRocket(now+480+Math.random()*500);}this.lastLaunch=now;}
    ...
    _finish(now){if(!['finishing','finale','manual'].includes(this.state))return;if(this.state==='finishing'){const drained=!this.rockets.length&&!this.pendingRockets.length&&!this.particles.length&&!this.textBlocks.length,waitLimit=this.wantFinale?Math.min(this.options.maxFinishTime,this.options.finale.maxWaitBeforeLaunch):this.options.maxFinishTime,timeout=now-this.finishStarted>=waitLimit;if(drained||timeout){if(timeout){this.rockets.length=0;this.pendingRockets.length=0;this.particles.length=0;this.flashes.length=0;this.textBlocks.length=0;}if(this.wantFinale&&!this.finalePlayed){this.state='finale';this.launchFinale({stopAfter:true});this.finaleStarted=now;}else this._fade(false);}}else if(this.state==='finale'){const done=!this.rockets.length&&!this.pendingRockets.length&&!this.particles.length&&!this.flashes.length&&!this.textBlocks.length,timeout=now-(this.finaleStarted||now)>this.options.finale.maxDuration;if(timeout)this._fade(false);else if(done){if(!this.finaleFinishedAt)this.finaleFinishedAt=now;if(now-this.finaleFinishedAt>=this.options.finale.finishDelay)this._fade(false);}else this.finaleFinishedAt=0;}else if(this.state==='manual'){const done=!this.rockets.length&&!this.pendingRockets.length&&!this.particles.length&&!this.flashes.length&&!this.textBlocks.length;if(done)this._fade(false);}}
    ...
    setOptions(partial={}){const previous=this.options;this.userOptions=merge(this.userOptions,partial);this.options=this._resolve(this.userOptions);if(this.root)this.root.style.opacity=String(clamp(Number(this.options.visuals.opacity??1),0,1));if(previous&&previous.performance.dprCap!==this.options.performance.dprCap&&this.renderer&&this.state!=='destroyed')this._resize();if(!this.options.performance.pauseWhenHidden)this._resumeFor('hidden');if(!this.options.performance.pauseWhenOffscreen)this._resumeFor('offscreen');return this;}
    setOpacity(level){return this.setOptions({visuals:{opacity:clamp(Number(level),0,1)}});}
    setStyle(name){const s=STYLES[name];if(!s)return this;this.setOptions({baseStyle:name,...s,colorTheme:this.options.colorTheme});if(this.root)this.root.style.opacity=String(clamp(Number(this.options.visuals.opacity??1),0,1));return this;}
    setColorTheme(name){const t=COLOR_THEMES[name];if(!t)return this;const style=STYLES[this.options.baseStyle]||STYLES.medium,palettes=t.palettes==='default'?style.show.palettes:t.palettes;this.setOptions({colorTheme:name,show:{palettes}});const palette=this._palette();if(palette&&palette.length){const colors=palette.map(h=>{const c=h.replace('#','');const n=parseInt(c.length===3?c.split('').map(x=>x+x).join(''):c,16);return[(n>>16&255)/255,(n>>8&255)/255,(n&255)/255];});this.particles.forEach((p,i)=>{const c=colors[i%colors.length];p.r=c[0];p.g=c[1];p.b=c[2];});this.rockets.forEach(r=>{r.colors=palette;});}return this;}
  ...
  GrandFireworks.VERSION='1.4.0';GrandFireworks.DEFAULTS=DEFAULTS;GrandFireworks.PRESETS=PRESETS;GrandFireworks.TYPES=TYPES;GrandFireworks.STYLES=STYLES;GrandFireworks.COLOR_THEMES=COLOR_THEMES;global.GrandFireworks=GrandFireworks;
})(window);
